...
/* 라인 아이콘 — 이모지 대체 (보타니칼 디자인 언어, stroke=currentColor) */
const IC = (p, vb) => `<svg class="ic-line" viewBox="${vb || "0 0 24 24"}" aria-hidden="true">${p}</svg>`;
const ICONS = {
  leaf: IC('<path d="M12 20 C12 12, 14 8, 20 4 C 20 12, 17 17, 12 20 Z"/><path d="M12 20 C 12 15, 10 12, 5 9 C 5 14, 8 18, 12 20"/>'),
  medal: IC('<circle cx="12" cy="9.5" r="5.5"/><path d="M9 14.3 L7.6 20.5 L12 18.2 L16.4 20.5 L15 14.3"/>'),
  sprout: IC('<path d="M12 21 V11.5"/><path d="M12 12 C 11.5 8.5, 9 6.5, 5.5 6.5 C 5.8 10, 8.5 12, 12 12 Z"/><path d="M12 10.5 C 12.3 7.5, 14.5 5.5, 17.8 5.5 C 17.6 8.8, 15.2 10.8, 12 10.5 Z"/>'),
  cup: IC('<path d="M5 9 H16.5 V15 C16.5 18, 14.5 20, 11.5 20 H10 C7 20, 5 18, 5 15 Z"/><path d="M16.5 10.5 H18.5 C20 10.5, 20.8 11.5, 20.8 12.6 C20.8 14, 19.8 15, 18.3 15 H16.5"/><path d="M8.5 6 C8.5 5, 9.4 4.7, 9.4 3.8 M12.5 6 C12.5 5, 13.4 4.7, 13.4 3.8"/>'),
  path: IC('<path d="M5 19.5 C 9.5 17.5, 9 12.5, 12 11.3 C 15 10, 15 6, 19 4.8"/><circle cx="5" cy="19.5" r="1.3"/><circle cx="19" cy="4.8" r="1.3"/>'),
  bars: IC('<path d="M5 20 V13 M10 20 V9 M15 20 V12 M20 20 V6"/>'),
  trendUp: IC('<path d="M4 17 L10 11 L13.5 14.5 L20 8"/><path d="M15.5 8 H20 V12.5"/>'),
...
const EMOTIONS = [
  ...
  { k: "신나요",   e: "🤩", base: "활기차요", en: 5, band: "pos", v: 88 }, // excited 8.0 → (8.0-1)/8×100=88 (동일 규준=동일 점수, false precision 배제)
  { k: "뿌듯해요", e: "😏", base: "괜찮아요", en: 4, band: "pos", v: 88 }, // proud 8.0 → 88 (신나요와 규준 동률 — 각성축(en)으로만 구분)
  ...
  { k: "무기력해요", e: "😶‍🌫️", base: "무기력해요", en: 1, band: "neg", v: 18 }, // helpless 2.4 → (2.4-1)/8×100=18 (동일 규준=동일 점수)
  { k: "외로워요",   e: "😔",   base: "우울해요",   en: 1, band: "neg", v: 18 }, // lonely 2.4 → 18 (무기력과 규준 동률)
...
function loadEntries() {
  try {
    const o = JSON.parse(localStorage.getItem(DB.ENTRIES)) || {};
    for (const k in o) { const e = o[k]; if (e && !Array.isArray(e.tags)) e.tags = (typeof e.tags === "string" && e.tags) ? [e.tags] : []; } // 손상·병합 데이터의 tags 비배열 방어(렌더 크래시 방지)
    return o;
  } catch { return {}; }
}
...
function renderOb() {
  ...
  document.getElementById("obEmoji").innerHTML = ICONS[s.e] || ICONS.leaf;
...
const tabs = { today: "tab-today", calendar: "tab-calendar", rest: "tab-rest", social: "tab-social", stats: "tab-stats", settings: "tab-settings" };
function activateTab(name, { scroll = true } = {}) {
  ...
  if (name === "social") { refreshSocial(); if (typeof medObserveStart === "function") medObserveStart(); } // 함께 탭 — 방 목록(캐시→서버) + 실시간 인원 관찰
  if (name !== "rest" && name !== "social" && typeof medObserveStop === "function") medObserveStop(); // 명상·함께 탭 벗어나면 방 관찰 종료
...
function openHabitDetail(id) {
  ...
  stowSetup();
  openHabitId = id; openSubpage(h.title, detailHabitHtml(h), "habit"); fillHabitGrid(h);
}
...
function refreshHabitDetail(id) {
  ...
  const h = loadChs().find((x) => x.id === id); if (!h) return;
  subTitle.textContent = h.title; subBody.innerHTML = detailHabitHtml(h); fillHabitGrid(h);
}
...
async function shareHabit(h) {
  const doneCount = habitDoneCount(h);
  const text = `오늘의 쉼 '${h.title}' 90일 챌린지 — ${doneCount}일 달성! 함께 해요`;
  try {
...
function confetti() {
  ...
  for (let i = 0; i < 28; i++) {
    ...
    s.className = "confetti petal p" + (i % 5);
    s.style.left = Math.random() * 100 + "vw"; s.style.animationDelay = (Math.random() * 0.4) + "s";
    const sc = 0.7 + Math.random() * 0.9; s.style.width = Math.round(11 * sc) + "px"; s.style.height = Math.round(15 * sc) + "px";
...
function calcStreakInfo(entries) {
  let streak = 0, freezesUsed = 0, d = new Date();
...
function calcStreak(entries) { return calcStreakInfo(entries).streak; }
...
// 주간 추세 — 앱 전체 단일 기준: 캘린더 최근 7일 vs 이전 7일 · 0-100 척도 · 각 기간 최소 3일 기록.
// 표본 미달이면 null — 1건 vs 1건으로 "▲40점" 같은 허위 추세를 만들지 않는다(카드·홈·리포트 공용).
function weekTrend(entries) {
  const grab = (from, to) => { const v = []; for (let i = from; i < to; i++) { const d = new Date(); d.setDate(d.getDate() - i); const s = entryScore(entries[todayKey(d)]); if (s != null) v.push(s); } return v; };
  const wk = grab(0, 7), pv = grab(7, 14);
  if (wk.length < 3 || pv.length < 3) return null;
  const avg = (a) => a.reduce((s, v) => s + v, 0) / a.length;
  return { cur: avg(wk), prev: avg(pv), delta: Math.round(avg(wk) - avg(pv)), nCur: wk.length, nPrev: pv.length };
}
...
function renderMoodCalendar(entries) {
  ...
  for (let d = 1; d <= days; d++) {
    ...
    const e = entries[key];
    const sc = entryScore(e); // mood 라벨 없이 score만 있는 기록도 달력에 색으로 반영
    const score = sc != null ? Math.min(4, Math.floor(sc / 20)) + 1 : 0;
    const isToday = key === tk;
    const future = key > tk;
    html += `<button class="cal-cell m${score} ${isToday ? "today" : ""}" ${future ? "disabled" : ""} data-cal="${key}" title="${e && e.mood ? e.mood : (sc != null ? sc + "점" : "")}">${d}</button>`;
  }
  wrap.innerHTML = html;
...
function drawReportCanvas(kind) {
  ...
  const habLines = ha ? ha.rows.slice(0, 3).map((r) => {
    ...
    return { name: r.h.title, rate: `${r.rate}%`, sub: bits.join(" ") };
  }) : [];
...
const REPORT_PAPERS = {
  ba: "행동활성화 (Mazzucchelli, Kane & Rees, 2010, J. of Positive Psychology)",
  ...
  sleep: "수면과 정서 (Baglioni et al., 2016, Psychological Bulletin)",
  ...
  plan: "문제해결치료 (Nezu, 2004, Behavior Therapy)",
...
function habitInsightLines(ha, unit) {
  if (!ha) return [];
  const nm = (r) => `<b>${escapeHtml(r.h.title)}</b>`;
...
function habitReportCard(ha, unit) {
  ...
  const rowsHtml = ha.rows.map((r) => {
    ...
    return `<div class="hrep-row">`
      + `<div class="hrep-line"><span class="hsum-name">${ICONS.leaf} ${escapeHtml(r.h.title)}</span>`
...
function reportDetailHtml(kind, off = 0) {
  ...
  const cmpOk = cur.days >= 3 && prev.days >= 3;
  const delta = (a, b) => (!cmpOk || a == null || b == null) ? null : a - b;
  ...
  const drows = keys.filter((k) => entries[k]).map((k) => { const e = entries[k], p = k.split("-"); return `<div class="rpt-row"><span>${+p[1]}/${+p[2]} (${dayOfWeekKo(k)})</span><span>${e.mood ? e.mood : "-"}</span><span>${entryScore(e) != null ? Math.round(entryScore(e)) + "점" : ""}</span></div>`; }).join("");
  ...
  { const mbd = {}; keys.forEach((k) => { const e = entries[k]; if (e && e.mood) mbd[k] = entryScore(e); });
    ...
    if (bH) hbits.push(`<b>${escapeHtml(bH.h.title)}</b> 한 날 기분이 더 좋았어요`); }
...
function renderBadges() {
  ...
  const _lvEl = document.getElementById("levelEmoji"); if (_lvEl) _lvEl.innerHTML = ICONS.sprout;
  ...
  if (nb) { const nextB = BADGES.find((b) => !earned.has(b.id)); nb.innerHTML = nextB ? `다음 배지 <b>${nextB.t}</b> · ${nextB.d}` : "더 이상 모을 배지가 없어요!"; }
  ...
  const badgeHtml = (b) => `<button class="badge ${earned.has(b.id) ? "earned" : "locked"}" data-bid="${b.id}" aria-label="${b.t} — ${b.d}"><span class="badge-emoji">${ICONS.medal}</span><span class="badge-title">${b.t}</span></button>`;
...
document.getElementById("badgeGrid").addEventListener("click", (e) => {
  ...
  if (d) { d.innerHTML = `<b>${b.t}</b> <span class="${got ? "bd-got" : "bd-no"}">${got ? "획득 ✓" : "아직"}</span> — ${b.d}`; d.classList.remove("pulse"); void d.offsetWidth; d.classList.add("pulse"); }
});
/* 습관 ↔ 기분 상관관계 */
...
function renderCorrelation(entries) {
  ...
  body.innerHTML = rows.map(({ h, ad, an, diff, d, ci, n }) => {
    ...
    return `<div class="corr-row">
      <div class="corr-top"><span>${ICONS.leaf} ${escapeHtml(h.title)}</span><span class="corr-diff ${up ? "up" : down ? "down" : ""}">${sign}${Math.round(Math.abs(diff))}</span></div>
...
function renderDiscoveries(entries, list) {
  ...
  if (trd && Math.abs(trd.delta) >= 5) { const up = trd.delta > 0; cards.push({ sal: 60 + Math.abs(trd.delta), icon: up ? ICONS.trendUp : ICONS.trendDown, title: up ? "최근 7일, 그 전보다 기분이 좋아졌어요" : "최근 7일은 조금 가라앉았어요", sub: up ? "잘 지내고 있다는 신호예요" : "스스로를 더 아껴줄 때예요", viz: barPair("이전 7일", trd.prev, "최근 7일", trd.cur), tone: up ? "good" : "soft" }); }
  ...
  chs.forEach((hh) => {
    ...
    if (done.length >= 5 && not.length >= 5) {
      ...
      if (diff >= 6) cards.push({ sal: 70 + diff, icon: ICONS.leaf, title: `${escapeHtml(hh.title)} 한 날, 기분이 더 높았어요`, sub: `한 날 평균 ${Math.round(ad)}점 · 안 한 날 ${Math.round(an)}점 · 관찰된 상관일 뿐 인과는 아니에요`, viz: barPair("안 한 날", an, "한 날", ad), tone: "good" });
    }
  });
  // 3) 평온한 요일×시간대 (표본 4+ · 그날 바로 남긴 기록만 — 나중에 수정한 기록의 시각 왜곡 배제)
  ...
  moods.forEach((e) => {
    ...
    if (!ts || !e.date) return;
    const td = new Date(ts); if (todayKey(td) !== e.date) return; // 그날 작성된 기록만 시간대 신뢰
    const h = td.getHours();
  ...
  if (slots.length) { slots.sort((x, y) => y.avg - x.avg); const t = slots[0], [di, bk] = t.k.split("|"); cards.push({ sal: 40 + (t.avg - 50), icon: ICONS.clock, title: `${days[di]}요일 ${bk}에 평온한 경향이 있어요`, sub: "이 시간을 나를 위해 비워두면 좋을 수 있어요", viz: chip(`${days[di]} ${bk} · 평균 ${Math.round(t.avg)}점 · ${t.n}회`, t.avg), tone: "good" }); }
  ...
  if (tags.length) { tags.sort((x, y) => y.n - x.n); const t = tags[0]; const em = emoByKey(t.t); const cv = em && em.v != null ? em.v : 50; cards.push({ sal: 25 + t.n, icon: ICONS.tag, title: `요즘 자주 느낀 감정은 '${escapeHtml(t.t)}'`, sub: `최근 30일 ${t.n}번 느꼈어요`, viz: chip(`#${escapeHtml(t.t)} · ${t.n}회`, cv), tone: "" }); }
  // 5) 꾸준함 — 보호(빈 하루 메움)가 쓰였으면 정직하게 표기
  const si = calcStreakInfo(entries); if (si.streak >= 3) cards.push({ sal: 30 + si.streak, icon: ICONS.sprout, title: `${si.streak}일 이어서 기록 중이에요${si.freezesUsed ? ` (보호 ${si.freezesUsed}회 포함)` : ""}`, sub: "꾸준함이 마음 회복의 가장 큰 힘이에요", viz: "", tone: "good" });
...
function renderHabitSummary() {
  ...
  rows.sort((a, b) => b.rate - a.rate);
  el.innerHTML = rows.map((r) => `<div class="hsum-row"><span class="hsum-name">${ICONS.leaf} ${escapeHtml(r.h.title)}</span>`
    + `<div class="hsum-bar-wrap"><div class="hsum-bar" style="width:${r.rate}%"></div></div>`
...
function checkBadges() {
  ...
  if (fresh.length) {
    ...
    if (newLevel > prevLevel) { // 목표 도달로 레벨업
      ...
      setTimeout(() => { if (typeof toast === "function") toast(`레벨 업! Lv.${lv.level} '${lv.cur.name}' 달성`); confetti(); }, 1500);
    }
...
function checkSafetySignals(entriesArg, listArg) {
  ...
  let fire = detectCrisis(recentNotes);
  if (!fire) {
    // 최근 14일 중 7일 이상 기록 & 평균이 위기 밴드(≤22), 또는 최근 5개 기록 모두 ≤25
    const sc = []; for (let i = 0; i < 14; i++) { const d = new Date(); d.setDate(d.getDate() - i); const s = entryScore(entries[todayKey(d)]); if (s != null) sc.push(s); }
    const avg = sc.length ? sc.reduce((a, b) => a + b, 0) / sc.length : null;
    const last5 = list.filter((e) => entryScore(e) != null).slice(-5).map((e) => entryScore(e));
    fire = (sc.length >= 7 && avg != null && avg <= 22) || (last5.length === 5 && last5.every((v) => v <= 25));
  }
  if (fire) { showSafety(); settings.lastSafetyNudge = todayKey(); saveSettingsObj(settings); }
}
// 데이터 기반 인사이트 메시지 목록 (여러 곳에서 재사용)
// 인사이트 규칙 — 전부 0-100 다이얼 점수(entryScore) 기준으로 통일(라벨 1-5점과 척도 혼용 금지)
function computeInsightMsgs(entries, list) {
  const withMood = list.filter((e) => e.mood);
  const msgs = [];

  // R1: 최근 5개 기록 중 4개 이상 저점(≤39) → 행동활성화 (3일 만에 '요즘' 단정하지 않게 창·문구 완화)
  const last5m = withMood.slice(-5);
  if (last5m.length >= 4 && last5m.filter((e) => entryScore(e) <= 39).length >= 4) {
    msgs.push("최근 며칠 마음이 무거웠나 봐요. 기분이 나아지길 기다리기보다, 아주 작은 행동 하나가 먼저 도움이 될 수 있어요. '쉼' 탭의 2분 미션을 하나 해볼까요? (행동활성화)");
  }

  // R2: 에너지 낮은데 기분은 보통 이상 → 소진 대응
  const lastBoth = list.slice(-3).filter((e) => e.energy && e.mood);
  if (lastBoth.length && lastBoth.every((e) => e.energy <= 2 && entryScore(e) >= 45)) {
  ...
  const byDow = {};
  withMood.forEach((e) => { const d = dayOfWeekKo(e.date); (byDow[d] = byDow[d] || []).push(entryScore(e)); });
  let worst = null;
  Object.entries(byDow).forEach(([d, arr]) => { if (arr.length < 3) return; const avg = arr.reduce((s, v) => s + v, 0) / arr.length; if (!worst || avg < worst.avg) worst = { d, avg }; });
  if (worst && worst.avg < 45) msgs.push(`'${worst.d}요일'에 유독 힘이 빠지는 편이에요. 그날엔 일정을 조금 비워두면 어때요?`);

  // R7: 감사 공백 + 기분 저조 → 감사 넛지
  const last5 = list.slice(-5);
  const noPraise = last5.length >= 3 && last5.every((e) => !e.praise);
  const lowRecent = withMood.slice(-3).some((e) => entryScore(e) <= 39);
...
function codeToRoom(code) {
  try {
    const raw = String(code).trim().replace(/^SHIM1-/, "").replace(/-/g, "+").replace(/_/g, "/");
...
function medRoomsRender() {
  ...
  const card = (r, mine) =>
    `<button class="med-room${mine ? " mr-mine" : ""}" data-room="${r.id}" role="listitem" aria-label="${escapeHtml(r.name)}${mine ? " (내 방)" : ""} — ${escapeHtml(r.mood)}, 호흡 ${r.pat.join("·")}초">` +
    ...
    `</button>`;
  const add = `<button class="med-room mr-add" data-room="new" role="listitem" aria-label="방 만들기">` +
      `<span class="mr-top"><b class="mr-name">＋ 방 만들기</b></span><small class="mr-mood">나만의 호흡으로</small></button>`;
  medRoomsEl.innerHTML = solo + MED_ROOMS.map((r) => card(r, false)).join("")
    + loadMyRooms().map((r) => card(r, true)).join("") + add;
}
function medRoomsSyncCounts(state) {
  ...
  document.querySelectorAll(".mr-live").forEach((el) => {
...
if (medRoomsEl) medRoomsEl.addEventListener("click", (e) => {
  ...
  Sound.tap(); Haptic.tap();
  if (b.dataset.room === "new") { openRoomSheet(); return; }
  openMedGuide(b.dataset.room === "solo" ? null : b.dataset.room);
});
medRoomsRender();
...
function roomSheetHtml() {
  ...
  const step = (k, label, v) => `<div class="rs-pat-row"><span class="rs-pat-label">${label}</span>` +
    ...
    `<b data-rpv="${k}">${v}</b>` +
    `<button type="button" class="med-step-btn sm" data-rp="${k}" data-d="1" aria-label="${label} 늘리기">＋</button><span class="med-pat-unit">초</span></div>`;
...
function openRoomSheet() {
  ...
  ov.onclick = (e) => {
    ...
    if (sb) { // 호흡 패턴 조절
      Sound.tap(); Haptic.tap();
      const k = sb.dataset.rp, d = +sb.dataset.d, i = k === "in" ? 0 : k === "hold" ? 1 : 2;
      const lo = k === "in" ? BR_MIN.in : k === "hold" ? BR_MIN.hold : BR_MIN.ex;
      _roomDraft.pat[i] = clampPat(_roomDraft.pat[i] + d, lo, BR_MAX);
      const out = ov.querySelector(`[data-rpv="${k}"]`); if (out) out.textContent = _roomDraft.pat[i];
      return;
    }
    const a = e.target.closest("[data-rsact]"); if (!a) return;
    Sound.tap();
    const act = a.dataset.rsact;
    if (act === "create") {
...
function closeRoomSheet() {
  const ov = document.getElementById("roomSheet"); if (!ov) return;
  ov.classList.remove("show"); ov.innerHTML = ""; syncAppInert();
}
...
function renderSocial() {
  ...
  const rows = _srvRooms.map((r) => srvRoomRow(r, myId)).concat(localOnly.map((r) =>
...
// 로그인 상태가 바뀌면(로그인/로그아웃) 함께 탭 갱신 + 내 방 서버 발행 재시도
window.addEventListener("cloud:auth", () => {
  const t = document.getElementById("tab-social");
  if (t && !t.hidden) refreshSocial();
});
...
const _SPK_OFF = IC('<path d="M4 10 V14 H7.5 L12 18 V6 L7.5 10 Z"/><path d="M16 9.5 L20.5 14.5 M20.5 9.5 L16 14.5"/>');
function medSyncAmbIcon() { if (medAmbBtn) medAmbBtn.innerHTML = (Sound.state.ambientType && Sound.state.ambientType !== "off") ? _SPK_ON : _SPK_OFF; }
...
function syncAppInert() {
  try {
    const rs = document.getElementById("roomSheet");
    const open = (!journey.hidden && journey.classList.contains("show")) || (typeof medOverlay !== "undefined" && medOverlay && !medOverlay.hidden) || (typeof breathOverlay !== "undefined" && breathOverlay && !breathOverlay.hidden) || !!(rs && rs.classList.contains("show")); // 여정은 닫힘 애니메이션(300ms) 동안 hidden이 늦게 걸려 show 클래스로 판단
...
document.addEventListener("keydown", (e) => {
  ...
  // 위에 떠 있는 순서대로 닫기: 방 만들기 시트 → 호흡(여정 위에도 뜸) → 명상 → 여정 → 서브페이지
  const _rs = document.getElementById("roomSheet");
  if (_rs && _rs.classList.contains("show")) closeRoomSheet();
